<?php
    ...
    {   
        ...
        function Header(){             
            ...
            $this->Cell(190, 5, utf8_decode("PRODUCTO MAS VENDIDO POR SECTOR"),0,1, 'C',0);                                          
            $this->SetFont('Amble-Regular','',10);                                                                             
    ...
    $total = 0;      
    if($_GET['val'] == 'Todas'){    
         $sql=pg_query("select productos.cod_productos,codigo,articulo, sum(cantidad::int) as cantidad, sum(detalle_factura_venta.total_venta::float),nombre_sector from factura_venta, detalle_factura_venta,productos,clientes,cliente_sector,sectores,rutas where detalle_factura_venta.cod_productos = productos.cod_productos and factura_venta.id_factura_venta = detalle_factura_venta.id_factura_venta and factura_venta.id_cliente = clientes.id_cliente and clientes.id_cliente = cliente_sector.id_cliente and cliente_sector.id_sector = sectores.id_sectores and sectores.id_sectores = rutas.id_ruta and fecha_actual between '".$_GET['inicio']."' and '".$_GET['fin']."' group by productos.cod_productos,nombre_sector order by cantidad desc limit 20 ");        
    }else{
        $sql=pg_query("select productos.cod_productos,codigo,articulo, sum(cantidad::int) as cantidad, sum(detalle_factura_venta.total_venta::float),nombre_sector from factura_venta, detalle_factura_venta,productos,clientes,cliente_sector,sectores,rutas where detalle_factura_venta.cod_productos = productos.cod_productos and factura_venta.id_factura_venta = detalle_factura_venta.id_factura_venta and factura_venta.id_cliente = clientes.id_cliente and clientes.id_cliente = cliente_sector.id_cliente and cliente_sector.id_sector = sectores.id_sectores and sectores.id_sectores = rutas.id_ruta and sectores.id_sectores = '".$_GET['id']."' and fecha_actual between '".$_GET['inicio']."' and '".$_GET['fin']."' group by productos.cod_productos,nombre_sector order by cantidad desc limit 20 ");        
    }
